...
interface TagSongJoinDao {
    ...
    @Query("SELECT DISTINCT songs.* FROM songs INNER JOIN tag_song_joins ON songs.id=tag_song_joins.songId WHERE tag_song_joins.tagId = :tagId")
    fun getSongsForTag(tagId: Long): Single<List<Song>>

    @Query("SELECT DISTINCT tags.* FROM tags INNER JOIN tag_song_joins ON tags.id=tag_song_joins.tagId WHERE tag_song_joins.songId = :songId")
